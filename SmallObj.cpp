...
SmallObj::SmallObj() {
	initSmallObj(0, 0, 0, 0, 0, 0, 0, 0);
}
SmallObj::SmallObj(float initx, float inity, float x1, float y1, float x2, float y2, float endx, float endy) {
	initSmallObj(initx, inity, x1, y1, x2, y2, endx, endy);
}


float SmallObj::toRadians(float degrees) { return (degrees * 2.0f * 3.14159f) / 360.0f; }

void SmallObj::setVertices(int num) {
	numVertices = num;
}


void SmallObj::readPointFileSmallObj(string f) {
	int counter = 0;
	float xt = 0;
	float yt = 0;
	string line = "";
...
void SmallObj::initSmallObj(float initx, float inity, float x1, float y1, float x2, float y2, float endx, float endy) {
	float x[4], y[4];



	// calculate first ring
	readPointFileSmallObj("SmallObj.txt");
...
int SmallObj::getNumVertices() { return numVertices; }
int SmallObj::getNumIndices() { return numIndices; }
std::vector<int> SmallObj::getIndices() { return indices; }
std::vector<glm::vec3> SmallObj::getVertices() { return vertices; }
std::vector<glm::vec2> SmallObj::getTexCoords() { return texCoords; }
std::vector<glm::vec3> SmallObj::getNormals() { return normals; }
std::vector<glm::vec3> SmallObj::getStangents() { return sTangents; }
std::vector<glm::vec3> SmallObj::getTtangents() { return tTangents; }