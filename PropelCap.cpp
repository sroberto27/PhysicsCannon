...
PropelCap::PropelCap() {
	initPropelCap(0, 0, 0, 0, 0, 0, 0, 0);
}
PropelCap::PropelCap(float initx, float inity, float x1, float y1, float x2, float y2, float endx, float endy) {
	initPropelCap(initx, inity, x1, y1, x2, y2, endx, endy);
}


float PropelCap::toRadians(float degrees) { return (degrees * 2.0f * 3.14159f) / 360.0f; }

void PropelCap::setVertices(int num) {
	numVertices = num;
}


void PropelCap::readPointFilePropelCap(string f) {
	int counter = 0;
	float xt = 0;
	float yt = 0;
	string line = "";
	...
	else cout << "Unable to open file  propelcap";
	...
	if (myfile2.is_open())
	{
		...
		cout << "Unable to open file  propelcap";
	//numVertices = counter;
...
void PropelCap::initPropelCap(float initx, float inity, float x1, float y1, float x2, float y2, float endx, float endy) {
	float x[4], y[4];



	// calculate first ring
	readPointFilePropelCap("PropelCap.txt");
...
int PropelCap::getNumVertices() { return numVertices; }
int PropelCap::getNumIndices() { return numIndices; }
std::vector<int> PropelCap::getIndices() { return indices; }
std::vector<glm::vec3> PropelCap::getVertices() { return vertices; }
std::vector<glm::vec2> PropelCap::getTexCoords() { return texCoords; }
std::vector<glm::vec3> PropelCap::getNormals() { return normals; }
std::vector<glm::vec3> PropelCap::getStangents() { return sTangents; }
std::vector<glm::vec3> PropelCap::getTtangents() { return tTangents; }