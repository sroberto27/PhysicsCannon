...
MidBooster::MidBooster() {
	initMidBooster(0, 0, 0, 0, 0, 0, 0, 0);
}
MidBooster::MidBooster(float initx, float inity, float x1, float y1, float x2, float y2, float endx, float endy) {
	initMidBooster(initx, inity, x1, y1, x2, y2, endx, endy);
}


float MidBooster::toRadians(float degrees) { return (degrees * 2.0f * 3.14159f) / 360.0f; }

void MidBooster::setVertices(int num) {
	numVertices = num;
}


void MidBooster::readPointFileMidBooster(string f) {
	int counter = 0;
	float xt = 0;
	float yt = 0;
	string line = "";
	...
	else cout << "Unable to open file  mid boster";
	...
	if (myfile2.is_open())
	{
		...
		cout << "Unable to open file  mid boster";
	//numVertices = counter;
...
void MidBooster::initMidBooster(float initx, float inity, float x1, float y1, float x2, float y2, float endx, float endy) {
	float x[4], y[4];



	// calculate first ring
	readPointFileMidBooster("MidBooster.txt");
...
int MidBooster::getNumVertices() { return numVertices; }
int MidBooster::getNumIndices() { return numIndices; }
std::vector<int> MidBooster::getIndices() { return indices; }
std::vector<glm::vec3> MidBooster::getVertices() { return vertices; }
std::vector<glm::vec2> MidBooster::getTexCoords() { return texCoords; }
std::vector<glm::vec3> MidBooster::getNormals() { return normals; }
std::vector<glm::vec3> MidBooster::getStangents() { return sTangents; }
std::vector<glm::vec3> MidBooster::getTtangents() { return tTangents; }