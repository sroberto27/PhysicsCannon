...
LuncherBase::LuncherBase() {
	initLuncherBase(0, 0, 0, 0, 0, 0, 0, 0);
}
LuncherBase::LuncherBase(float initx, float inity, float x1, float y1, float x2, float y2, float endx, float endy) {
	initLuncherBase(initx, inity, x1, y1, x2, y2, endx, endy);
}


float LuncherBase::toRadians(float degrees) { return (degrees * 2.0f * 3.14159f) / 360.0f; }

void LuncherBase::setVertices(int num) {
	numVertices = num;
}


void LuncherBase::readPointFileLuncherBase(string f) {
	int counter = 0;
	float xt = 0;
	float yt = 0;
	string line = "";
	...
	else cout << "Unable to open file luncher";
	...
	if (myfile2.is_open())
	{
		...
		cout << "Unable to open file luncher";
	//numVertices = counter;
	prec = (outer - inner) * 100;
	numVertices = (prec + 1) * (prec + 1);
	numIndices = prec * prec * 6;
	//prec = (outer- inner)*1000;
	cout << "pass by lunchbase" << endl;
...
void LuncherBase::initLuncherBase(float initx, float inity, float x1, float y1, float x2, float y2, float endx, float endy) {
	float x[4], y[4];



	// calculate first ring
	readPointFileLuncherBase("LuncherBase.txt");
...
int LuncherBase::getNumVertices() { return numVertices; }
int LuncherBase::getNumIndices() { return numIndices; }
std::vector<int> LuncherBase::getIndices() { return indices; }
std::vector<glm::vec3> LuncherBase::getVertices() { return vertices; }
std::vector<glm::vec2> LuncherBase::getTexCoords() { return texCoords; }
std::vector<glm::vec3> LuncherBase::getNormals() { return normals; }
std::vector<glm::vec3> LuncherBase::getStangents() { return sTangents; }
std::vector<glm::vec3> LuncherBase::getTtangents() { return tTangents; }