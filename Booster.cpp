...
Booster::Booster() {
	initBooster(0, 0, 0, 0, 0, 0, 0, 0);
}
Booster::Booster(float initx, float inity, float x1, float y1, float x2, float y2, float endx, float endy) {
	initBooster(initx, inity, x1, y1, x2, y2, endx, endy);
}


float Booster::toRadians(float degrees) { return (degrees * 2.0f * 3.14159f) / 360.0f; }

void Booster::setVertices(int num) {
	numVertices = num;
}


void Booster::readPointFileBooster(string f) {
	int counter = 0;
	float xt = 0;
	float yt = 0;
	string line = "";
	...
	else cout << "Unable to open file bostetr";
	...
	if (myfile2.is_open())
	{
		while (getline(myfile2, line)) {
			...
			cout << " temBOOsterrrVErtread x" << xt << "  y  " << yt << endl;
		...
		cout << "Unable to open file boster";
	//numVertices = counter;
...
void Booster::initBooster(float initx, float inity, float x1, float y1, float x2, float y2, float endx, float endy) {
	float x[4], y[4];



	// calculate first ring
	readPointFileBooster("Booster.txt");
...
int Booster::getNumVertices() { return numVertices; }
int Booster::getNumIndices() { return numIndices; }
std::vector<int> Booster::getIndices() { return indices; }
std::vector<glm::vec3> Booster::getVertices() { return vertices; }
std::vector<glm::vec2> Booster::getTexCoords() { return texCoords; }
std::vector<glm::vec3> Booster::getNormals() { return normals; }
std::vector<glm::vec3> Booster::getStangents() { return sTangents; }
std::vector<glm::vec3> Booster::getTtangents() { return tTangents; }