...
Ground::Ground() {
	initGround(0, 0, 0, 0, 0, 0, 0, 0);
}
Ground::Ground(float initx, float inity, float x1, float y1, float x2, float y2, float endx, float endy) {
	initGround(initx, inity, x1, y1, x2, y2, endx, endy);
}


float Ground::toRadians(float degrees) { return (degrees * 2.0f * 3.14159f) / 360.0f; }

void Ground::setVertices(int num) {
	numVertices = num;
}


void Ground::readPointFileGround(string f) {
	int counter = 0;
	float xt = 0;
	float yt = 0;
	string line = "";
...
void Ground::initGround(float initx, float inity, float x1, float y1, float x2, float y2, float endx, float endy) {
	float x[4], y[4];



	// calculate first ring
	readPointFileGround("Flat.txt");
...
int Ground::getNumVertices() { return numVertices; }
int Ground::getNumIndices() { return numIndices; }
std::vector<int> Ground::getIndices() { return indices; }
std::vector<glm::vec3> Ground::getVertices() { return vertices; }
std::vector<glm::vec2> Ground::getTexCoords() { return texCoords; }
std::vector<glm::vec3> Ground::getNormals() { return normals; }
std::vector<glm::vec3> Ground::getStangents() { return sTangents; }
std::vector<glm::vec3> Ground::getTtangents() { return tTangents; }