...
WindBooster::WindBooster() {
	initWindBooster(0, 0, 0, 0, 0, 0, 0, 0);
}
WindBooster::WindBooster(float initx, float inity, float x1, float y1, float x2, float y2, float endx, float endy) {
	initWindBooster(initx, inity, x1, y1, x2, y2, endx, endy);
}


float WindBooster::toRadians(float degrees) { return (degrees * 2.0f * 3.14159f) / 360.0f; }

void WindBooster::setVertices(int num) {
	numVertices = num;
}


void WindBooster::readPointFileWindBooster(string f) {
	int counter = 0;
	float xt = 0;
	float yt = 0;
	string line = "";
	...
	else cout << "Unable to open file  windBoster";
	...
	if (myfile2.is_open())
	{
		...
		cout << "Unable to open file  windBoster";
	//numVertices = counter;
...
void WindBooster::initWindBooster(float initx, float inity, float x1, float y1, float x2, float y2, float endx, float endy) {
	float x[4], y[4];



	// calculate first ring
	readPointFileWindBooster("WindBooster.txt");
...
int WindBooster::getNumVertices() { return numVertices; }
int WindBooster::getNumIndices() { return numIndices; }
std::vector<int> WindBooster::getIndices() { return indices; }
std::vector<glm::vec3> WindBooster::getVertices() { return vertices; }
std::vector<glm::vec2> WindBooster::getTexCoords() { return texCoords; }
std::vector<glm::vec3> WindBooster::getNormals() { return normals; }
std::vector<glm::vec3> WindBooster::getStangents() { return sTangents; }
std::vector<glm::vec3> WindBooster::getTtangents() { return tTangents; }