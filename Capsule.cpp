...
Capsule::Capsule() {
	initCapsule(0, 0, 0, 0, 0, 0, 0, 0);
}
Capsule::Capsule(float initx, float inity, float x1, float y1, float x2, float y2, float endx, float endy) {
	initCapsule(initx, inity, x1, y1, x2, y2, endx, endy);
}


float Capsule::toRadians(float degrees) { return (degrees * 2.0f * 3.14159f) / 360.0f; }

void Capsule::setVertices(int num) {
	numVertices = num;
}


void Capsule::readPointFileCapsule(string f) {
	int counter = 0;
	float xt = 0;
	float yt = 0;
	string line = "";
	...
	else cout << "Unable to open file capsule";
	...
	if (myfile2.is_open())
	{
		...
		cout << "Unable to open file capsule";
	//numVertices = counter;
...
void Capsule::initCapsule(float initx, float inity, float x1, float y1, float x2, float y2, float endx, float endy) {
	float x[4], y[4];



	// calculate first ring
	readPointFileCapsule("Capsule.txt");
...
int Capsule::getNumVertices() { return numVertices; }
int Capsule::getNumIndices() { return numIndices; }
std::vector<int> Capsule::getIndices() { return indices; }
std::vector<glm::vec3> Capsule::getVertices() { return vertices; }
std::vector<glm::vec2> Capsule::getTexCoords() { return texCoords; }
std::vector<glm::vec3> Capsule::getNormals() { return normals; }
std::vector<glm::vec3> Capsule::getStangents() { return sTangents; }
std::vector<glm::vec3> Capsule::getTtangents() { return tTangents; }