...
WindMidBooster::WindMidBooster() {
	initWindMidBooster(0, 0, 0, 0, 0, 0, 0, 0);
}
WindMidBooster::WindMidBooster(float initx, float inity, float x1, float y1, float x2, float y2, float endx, float endy) {
	initWindMidBooster(initx, inity, x1, y1, x2, y2, endx, endy);
}


float WindMidBooster::toRadians(float degrees) { return (degrees * 2.0f * 3.14159f) / 360.0f; }

void WindMidBooster::setVertices(int num) {
	numVertices = num;
}


void WindMidBooster::readPointFileWindMidBooster(string f) {
	int counter = 0;
	float xt = 0;
	float yt = 0;
	string line = "";
	...
	else cout << "Unable to open file  windMidBoster";
	...
	if (myfile2.is_open())
	{
		...
		cout << "Unable to open file  windMidBoster";
	//numVertices = counter;
...
void WindMidBooster::initWindMidBooster(float initx, float inity, float x1, float y1, float x2, float y2, float endx, float endy) {
	float x[4], y[4];



	// calculate first ring
	readPointFileWindMidBooster("WindMidBooster.txt");
...
int WindMidBooster::getNumVertices() { return numVertices; }
int WindMidBooster::getNumIndices() { return numIndices; }
std::vector<int> WindMidBooster::getIndices() { return indices; }
std::vector<glm::vec3> WindMidBooster::getVertices() { return vertices; }
std::vector<glm::vec2> WindMidBooster::getTexCoords() { return texCoords; }
std::vector<glm::vec3> WindMidBooster::getNormals() { return normals; }
std::vector<glm::vec3> WindMidBooster::getStangents() { return sTangents; }
std::vector<glm::vec3> WindMidBooster::getTtangents() { return tTangents; }