...
Pedestal::Pedestal() {
	initPedestal(0, 0, 0, 0, 0, 0, 0, 0);
}
Pedestal::Pedestal(float initx, float inity, float x1, float y1, float x2, float y2, float endx, float endy) {
	initPedestal(initx, inity, x1, y1, x2, y2, endx, endy);
}


float Pedestal::toRadians(float degrees) { return (degrees * 2.0f * 3.14159f) / 360.0f; }

void Pedestal::setVertices(int num) {
	numVertices = num;
}


void Pedestal::readPointFilePedestal(string f) {
	int counter = 0;
	float xt = 0;
	float yt = 0;
	string line = "";
...
void Pedestal::initPedestal(float initx, float inity, float x1, float y1, float x2, float y2, float endx, float endy) {
	float x[4], y[4];



	// calculate first ring
	readPointFilePedestal("Pedestal.txt");
...
int Pedestal::getNumVertices() { return numVertices; }
int Pedestal::getNumIndices() { return numIndices; }
std::vector<int> Pedestal::getIndices() { return indices; }
std::vector<glm::vec3> Pedestal::getVertices() { return vertices; }
std::vector<glm::vec2> Pedestal::getTexCoords() { return texCoords; }
std::vector<glm::vec3> Pedestal::getNormals() { return normals; }
std::vector<glm::vec3> Pedestal::getStangents() { return sTangents; }
std::vector<glm::vec3> Pedestal::getTtangents() { return tTangents; }